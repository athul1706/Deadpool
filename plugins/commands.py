...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.reply(script.STRT_TXT.format(message.from_user.mention if message.from_user else message.chat.title, temp.U_NAME, temp.B_NAME), reply_markup=reply_markup)
    ...
    if len(message.command) != 2:
        ...
        await message.reply_photo(
            ...
            caption=script.START_TXT.format(greeting, message.from_user.mention, uptime),
...
async def request_moviez_file(bot, message):   
    if len(message.command) < 2:
    ...
    buttons = [
        [
            InlineKeyboardButton('UPLOADED',
                                 callback_data=f'rekast#updated#{message.id}#{message.from_user.id}')
        ],
        [
        ...
        [
            InlineKeyboardButton('FILES NOT FOUND',
                                 callback_data=f'rekast#notfound#{message.id}#{message.from_user.id}')
        ],
    ]
    reply_markup = InlineKeyboardMarkup(buttons)
