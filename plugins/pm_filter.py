...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        size = get_size(files.file_size)
        f_caption = files.caption
        file_format = title.split('.')[-1]  # Get the file format
        f_caption_parts = f_caption.split('.')
        if file_format.lower() in f_caption_parts:
            f_caption_parts = f_caption_parts[:f_caption_parts.index(file_format.lower()) + 1]
        f_caption = ' '.join(f_caption_parts)
