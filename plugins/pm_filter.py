...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("athul"):              
        ident, file_id = query.data.split("#")
        athull_ = await get_file_details(file_id)
        if not athull_:
    ...
    if query.data.startswith("file"):
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.START_TXT.format(greeting, query.from_user.mention, uptime),
        ...
        buttons = [[
                ...
                InlineKeyboardButton('ᴀᴅᴍɪɴ ᴍᴏᴅ', callback_data='admin'),
                InlineKeyboardButton('ғɪʟᴇ sᴛᴏʀᴇ', callback_data='fstore')
        ],[
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⏹️ ғᴏʀᴍᴀᴛᴛɪɴɢ', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            InlineKeyboardButton('🔙 ʙᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('👩‍💻 ᴏᴡɴᴇʀ ᴍᴏᴅ', callback_data='owner')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def pmauto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        message = msg.message.reply_to_message  # msg will be callback query
        search, files, offset, total_results = spoll
    pre = 'filep' if settings['file_secure'] else 'athul'
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
...
async def nextp_page(bot, query):
    ...
    if settings['button']:
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{file.file_name}", callback_data=f'athul#{file.file_id}'
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    callback_data=f'athull_#{file.file_id}',
                ),
            ]
            for file in files
        ]
...
async def advantage_spoll_chec(bot, query):
    _, user, movie_ = query.data.split('#')
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("okDa", show_alert=True)
    if movie_ == "close_spellcheck":
    ...
    if k == False:
        ...
        if files:
            k = (movie, files, offset, total_results)
            await pmauto_filter(bot, query, k)
        else:
            k = await query.message.edit('This Movie Not Found In DataBase')
            await asyncio.sleep(10)
            await k.delete()

async def advantage_spell_check(bot, msg):
    query = re.sub(
        r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|((send|snd|giv(e)?|gib)(\sme)?)|movie(s)?|new|latest|br((o|u)h?)*|^h(e|a)?(l)*(o)*|mal(ayalam)?|t(h)?amil|file|that|find|und(o)*|kit(t(i|y)?)?o(w)?|thar(u)?(o)*w?|kittum(o)*|aya(k)*(um(o)*)?|full\smovie|any(one)|with\ssubtitle(s)?)",
        "", msg.text, flags=re.IGNORECASE)  # plis contribute some common words
    query = query.strip() + " movie"
    ...
    movielist = list(dict.fromkeys(movielist))  # removing duplicates
    if not movielist:
        k = await bot.send_message("I couldn't find anything related to that. Check your spelling")
        await asyncio.sleep(8)
        await k.delete()
        return
    SPELL_CHECK[msg.id] = movielist
    btn = [[
        InlineKeyboardButton(
            text=movie.strip(),
            callback_data=f"spollingpm#{user}#{k}",
        )
    ] for k, movie in enumerate(movielist)]
    btn.append([InlineKeyboardButton(text="Close", callback_data=f'spollingpm#{user}#close_spellcheck')])
    await bot.send_message("I couldn't find anything related to that\nDid you mean any one of these?",
